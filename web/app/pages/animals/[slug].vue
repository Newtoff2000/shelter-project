...
const ageGroup = computed(() => getAgeGroup(animal.value.ageYears))
const timeAtShelter = computed(() => getTimeAtShelter(animal.value.dateJoined))

const interestSubject = computed(() => `I'm interested in ${animal.value?.name}`)
</script>

<template>
  <div v-if="animal" class="profile">
    ...
    <div class="content">
      ...
      <section class="card">
        ...
        <dl class="info-grid">
          ...
          <div>
            <dt>Size</dt>
            <dd>{{ animal.size.charAt(0).toUpperCase() + animal.size.slice(1) }}</dd>
          </div>
          <div>
            <dt>At shelter</dt>
      ...
      <section class="card cta-card">
        ...
        <a :href="`/#contact?subject=${encodeURIComponent(interestSubject)}`" class="btn-primary">
      ...
      <!-- Personality -->
      <section v-if="animal.personality?.en || animal.personality?.pt" class="card">
        <h2>Personality</h2>
        <SanityContent :blocks="animal.personality.en ?? animal.personality.pt" />
      </section>

      <!-- History -->
      <section v-if="animal.history?.en || animal.history?.pt" class="card">
        <h2>History</h2>
        <SanityContent :blocks="animal.history.en ?? animal.history.pt" />
      </section>

      <!-- Health -->
      <section v-if="animal.health?.en || animal.health?.pt" class="card">
        <h2>Health</h2>
        <SanityContent :blocks="animal.health.en ?? animal.health.pt" />
      </section>

      <!-- Interesting facts -->
      <section v-if="animal.interestingFacts?.en || animal.interestingFacts?.pt" class="card">
        <h2>Interesting facts</h2>
        <SanityContent :blocks="animal.interestingFacts.en ?? animal.interestingFacts.pt" />
      </section>
